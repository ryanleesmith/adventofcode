class Tile:
    ...
    def flip(self):
        blackNeighbors = sum(map(lambda neighbor: neighbor.color, filter(lambda neighbor: neighbor, self.neighbors.values())))
        if self.color and (blackNeighbors == 0 or blackNeighbors > 2):
            return True
        elif not self.color and blackNeighbors == 2:
            return True
        return False
